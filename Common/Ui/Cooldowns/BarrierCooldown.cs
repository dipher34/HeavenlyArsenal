...
public class EnrageCooldown: CooldownHandler
{
    ...
    private float AdjustedCompletion => (instance.player.GetModPlayer<ShintoArmorPlayer>().enrageCooldown / ShintoArmorPlayer.EnrageCooldownMax)/60;
    public override void ApplyBarShaders(float opacity)
    {
        // Use the adjusted completion
        GameShaders.Misc["CalamityMod:CircularBarShader"].UseOpacity(opacity);
...
public class EnrageTimerVisual: CooldownHandler
{
    ...
    private float AdjustedCompletion => instance.timeLeft / ShintoArmorPlayer.EnrageCooldownMax;
    public override void ApplyBarShaders(float opacity)
    {
        // Use the adjusted completion
        GameShaders.Misc["CalamityMod:CircularBarShader"].UseOpacity(opacity);
