...
public class RocheLimitBlackHoleRenderer : ModSystem
{
    ...
    {
        int blackHoleID = ModContent.ProjectileType<RocheLimitProjectile>();
        foreach (Projectile blackHole in Main.ActiveProjectiles)
        {
            ...
            if (blackHole.type == blackHoleID)
                blackHole.As<RocheLimitProjectile>().RenderBlackHole();
        ...
        if (blackHoleTarget.TryGetTarget(0, out RenderTarget2D target) && target is not null)
        {
            ...
            foreach (Projectile blackHole in Main.ActiveProjectiles)
            {
                if (blackHole.type == blackHoleID)
                {
                    ...
                    {
                        blackHoleRadii[index] = blackHole.As<RocheLimitProjectile>().BlackHoleDiameter / WotGUtils.ViewportSize.X * Main.GameViewMatrix.Zoom.X * 0.275f;
