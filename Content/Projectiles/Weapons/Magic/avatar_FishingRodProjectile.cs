    ...
    public void UpdateBellRinging()
    {
        ...
        if (oldBellVelocity.Distance(currentBellVelocity) > 16 && currentBellVelocity.Length() > 10) // Finding large accelerations, indicating sudden movements that would ring the bell
        {
            if (BellRingCooldown <= 0)
            {
                ...
                SoundEngine.PlaySound(GennedAssets.Sounds.NamelessDeity.ClockStrike with { Pitch = 1f, MaxInstances = 0 }, bellPosition);

                if (!riftOpen)
                {
                    riftOpen = true;
                    SoundEngine.PlaySound(GennedAssets.Sounds.Avatar.RiftOpen with { MaxInstances = 0 }, Player.MountedCenter);
                }
            }
        }
