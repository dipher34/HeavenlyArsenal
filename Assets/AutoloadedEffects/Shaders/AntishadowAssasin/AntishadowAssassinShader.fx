...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float eyeOffsetNoise = tex2D(noiseTexture, coords * 0.4 + colorData.r + globalTime) * eyeInUse;
    float eyeCenterFade = smoothstep(0.1, 0.67, colorData.b) * smoothstep(1, 0.5, colorData.b * eyeScale);
    float4 eyeColor = float4(PaletteLerp(cos(globalTime * 4 + eyeOffsetNoise * 2.5) * 0.5 + 0.5), 1) * 2;
